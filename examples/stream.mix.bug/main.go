...
func main() {
	...
	src := p.AddSource("Fake Source").Trace()
	sp := src.AddProcessor(opts, sources.NewFaker(1, nil))
	...
	snk1.AddProcessor(opts, canalSnk.NewStdoutSink(), "path3")
	//snk1.AddProcessor(opts, canalSnk.NewBlackholeSink(), "path3")

	snk2 := p.AddSink("Enrich Sink")
	snk2.AddProcessor(opts, canalSnk.NewStdoutSink(), "path4")
